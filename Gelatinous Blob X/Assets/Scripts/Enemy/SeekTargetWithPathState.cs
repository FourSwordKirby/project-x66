...
public class SeekTargetWithPathState : State<Enemy> {
    ...
    private const float DEFAULT_ARRIVE_RADIUS = 0.3f;
    ...
    public SeekTargetWithPathState(Enemy enemy, StateMachine<Enemy> fsm, GameObject target, float arriveRadius = DEFAULT_ARRIVE_RADIUS)
        : base(enemy, fsm)
    {
        this.target = target;
    ...
    public bool IsAtTarget()
    {
        ...
        return Vector3.Distance(targetPos, selfPos) < arriveRadius;
    }
    ...
    {
        Owner.NavAgent.SetDestination(target.transform.position);
        if (target.GetComponent<Player>() != null)
        {
            if (!Owner.targetingCone.GetComponentInChildren<Collider>().bounds.Intersects(target.GetComponentInChildren<Collider>().bounds))
        ...
        if(Vector3.Distance(targetPos, selfPos) < arriveRadius)
        {
            Vector3 interp = Vector3.Lerp(targetVelocity, Vector3.zero * Owner.speed, 0.2f);
            targetVelocity.x = interp.x;
            targetVelocity.z = interp.z;
            return;
        }
        ...
        else
        {
            ...
            else
            {
                ...
                if (Vector3.Distance(selfPos, pathPointTarget) < arriveRadius)
                {
                    pathIndex++;
        ...
        Vector3 arriveVelocity = StaticMovementAlgorithms.KinematicArrive(selfBody, arrivePoint, 1.0f, arriveRadius);
        ...
        {
            Vector3 interp = Vector3.Lerp(targetVelocity, arriveVelocity * Owner.speed, 0.2f);
            if (Mathf.Abs(arriveVelocity.y) > 1.0f)
            {
                targetVelocity = interp;
            }
            else
            {
                targetVelocity.x = interp.x;
